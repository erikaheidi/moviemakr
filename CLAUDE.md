...
```bash
./moviemakr.py render scripts/example.yaml --dry-run   # print docker commands, render nothing
...
| `layout.py` | `slugify`, `RunLayout` (incl. `to_container`) | errors |
...
| `cli.py` | argparse, `cmd_status` / `cmd_assemble` | everything |
...
- `<project>/assets` → `/assets` (read-only)
- `renders/<script-name>` → `/out` (read-write)
...
## Output layout (`renders/<script-name>/`)

`scenes/` raw model output · `frames/` last frame per scene (for chaining) ·
...
See `scripts/example.yaml` for the fully commented template: `model` paths,
